...
<head>
    ...
    <title>Edit Beban Operasional - Oriya Herbal Sejahtera</title>
    ...
    <script>
        ...
        // Update nama akun ketika kode akun dipilih
        function updateNamaAkun() {
            const select = document.getElementById('kode_akun');
            const namaAkunInput = document.getElementById('nama_akun');
            const selectedOption = select.options[select.selectedIndex];
            
            if (selectedOption.value) {
                namaAkunInput.value = selectedOption.getAttribute('data-nama-akun');
            } else {
                namaAkunInput.value = '';
            }
        }
    ...
    <div class="flex h-screen overflow-hidden">
        ...
        <div class="flex-1 flex flex-col overflow-hidden">
            ...
            <main class="flex-1 overflow-y-auto bg-white p-6">
                <div class="max-w-2xl">
                    <h2 class="text-3xl font-bold text-[#2d5016] mb-6">Edit Beban Operasional</h2>
                    ...
                    <form action="{{ route('beban-operasional.update', $beban->id) }}" method="POST" class="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
                        @csrf
                        @method('PUT')

                        <div class="mb-4">
                            <label for="kode_akun" class="block text-sm font-medium text-gray-700 mb-2">Kode Beban</label>
                            <select name="kode_akun" id="kode_akun" onchange="updateNamaAkun()"
                                ...
                                @foreach($coas as $coa)
                                    <option value="{{ $coa->kode_akun }}" data-nama-akun="{{ $coa->nama_akun }}"
                                        {{ old('kode_akun', $beban->kode_akun) == $coa->kode_akun ? 'selected' : '' }}>
                        ...
                        <div class="mb-4">
                            ...
                            <input type="text" name="nama_akun" id="nama_akun" value="{{ old('nama_akun', $beban->nama_akun) }}" 
                                class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#90EE90] focus:border-transparent" 
                                readonly>
                        </div>
                        ...
                        <div class="mb-4">
                            ...
                            <input type="number" name="nominal" id="nominal" value="{{ old('nominal', $beban->nominal) }}" step="0.01" min="0"
                                class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#90EE90] focus:border-transparent" 
                                required>
                        </div>
                        ...
                        <div class="mb-6">
                            ...
                            <input type="date" name="tanggal_pengeluaran" id="tanggal_pengeluaran" value="{{ old('tanggal_pengeluaran', $beban->tanggal_pengeluaran->format('Y-m-d')) }}" 
                                class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#90EE90] focus:border-transparent" 
                                required>
                        </div>
                        ...
                        <div class="flex items-center gap-4">
                            ...
                            <a href="{{ route('beban-operasional.index') }}" class="bg-gray-200 hover:bg-gray-300 text-gray-700 px-6 py-2 rounded-lg font-semibold transition duration-200">
                                Batal
                            </a>
                        </div>
                    </form>

...
<head>
    ...
    <title>Daftar Beban - Oriya Herbal Sejahtera</title>
            ...
            <nav class="flex-1 overflow-y-auto p-4">
                <ul class="space-y-1">
                    ...
                    <li>
                        ...
                        <ul id="transaksi" class="mt-1 ml-8 space-y-1">
                            <li>
                            ...
                            <li>
                                <a href="{{ route('beban-operasional.index') }}" class="block px-4 py-2 text-sm bg-[#90EE90] text-[#2d5016] rounded-lg font-semibold">Beban Operasional</a>
                            </li>
                        </ul>
                    </li>
        ...
        <div class="flex-1 flex flex-col overflow-hidden">
            ...
            <main class="flex-1 overflow-y-auto bg-white p-6">
                <div class="flex items-center justify-between mb-6">
                    <h2 class="text-3xl font-bold text-[#2d5016]">Daftar Beban</h2>
                    <a href="{{ route('beban-operasional.create') }}" class="bg-[#90EE90] hover:bg-[#7ED87E] text-[#2d5016] px-4 py-2 rounded-lg font-semibold transition duration-200">
                        Tambah Beban
                    </a>
                </div>
                    ...
                    <table class="min-w-full divide-y divide-gray-200">
                        <thead class="bg-[#E8F5E9]">
                            <tr>
                                ...
                                <th class="px-6 py-3 text-left text-xs font-medium text-[#2d5016] uppercase tracking-wider">Kode Beban</th>
                                <th class="px-6 py-3 text-left text-xs font-medium text-[#2d5016] uppercase tracking-wider">Nama Akun</th>
                                <th class="px-6 py-3 text-left text-xs font-medium text-[#2d5016] uppercase tracking-wider">Nominal</th>
                                <th class="px-6 py-3 text-left text-xs font-medium text-[#2d5016] uppercase tracking-wider">Tanggal Pengeluaran</th>
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            @forelse($bebans as $index => $beban)
                                <tr>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{{ $beban->kode_akun }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{{ $beban->nama_akun }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">Rp {{ number_format($beban->nominal, 0, ',', '.') }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{{ $beban->tanggal_pengeluaran->format('d/m/Y') }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                        <div class="flex items-center gap-2">
                                            <a href="{{ route('beban-operasional.edit', $beban->id) }}" class="text-[#2d5016] hover:text-[#90EE90] transition duration-200">Edit</a>
                                            <span class="text-gray-300">|</span>
                                            <form id="delete-form-{{ $beban->id }}" action="{{ route('beban-operasional.destroy', $beban->id) }}" method="POST" class="inline">
                                                @csrf
                                                @method('DELETE')
                                                <button type="button" onclick="confirmDelete({{ $beban->id }})" class="text-red-600 hover:text-red-800 transition duration-200">Hapus</button>
                                            </form>
                                        </div>
                                    </td>
                                </tr>
                            @empty
                                <tr>
                                    <td colspan="6" class="px-6 py-4 text-center text-sm text-gray-500">Tidak ada data beban operasional</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>

...
<head>
    ...
    <title>Kelola Data Produk - Oriya Herbal Sejahtera</title>
                ...
                <ul class="space-y-1">
                    ...
                    <li>
                        ...
                        <ul id="master-data" class="mt-1 ml-8 space-y-1">
                            ...
                            <li>
                                <a href="{{ route('produk.index') }}" class="block px-4 py-2 text-sm bg-[#90EE90] text-[#2d5016] rounded-lg font-semibold">Produk</a>
                            </li>
                            <li>
        ...
        <div class="flex-1 flex flex-col overflow-hidden">
            ...
            <main class="flex-1 overflow-y-auto bg-white p-6">
                <div class="flex items-center justify-between mb-6">
                    <h2 class="text-3xl font-bold text-[#2d5016]">Kelola Data Produk</h2>
                    <a href="{{ route('produk.create') }}" class="bg-[#90EE90] hover:bg-[#7ED87E] text-[#2d5016] px-4 py-2 rounded-lg font-semibold transition duration-200">
                        Tambah Produk
                    </a>
                </div>
                ...
                <div class="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
                    <table class="min-w-full divide-y divide-gray-200">
                        <thead class="bg-[#E8F5E9]">
                            <tr>
                                <th class="px-6 py-3 text-left text-xs font-medium text-[#2d5016] uppercase tracking-wider">Kode Produk</th>
                                <th class="px-6 py-3 text-left text-xs font-medium text-[#2d5016] uppercase tracking-wider">Nama Produk</th>
                                <th class="px-6 py-3 text-left text-xs font-medium text-[#2d5016] uppercase tracking-wider">Satuan</th>
                                <th class="px-6 py-3 text-left text-xs font-medium text-[#2d5016] uppercase tracking-wider">Harga Jual</th>
                                <th class="px-6 py-3 text-left text-xs font-medium text-[#2d5016] uppercase tracking-wider">Harga Beli</th>
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            @forelse($produks as $produk)
                                <tr>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{{ $produk->satuan }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">Rp {{ number_format($produk->harga_jual, 0, ',', '.') }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{{ $produk->harga_beli ? 'Rp ' . number_format($produk->harga_beli, 0, ',', '.') : '-' }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                        <div class="flex items-center gap-2">
                                            <a href="{{ route('produk.edit', $produk->id) }}" class="text-[#2d5016] hover:text-[#90EE90] transition duration-200">Edit</a>
                                            <span class="text-gray-300">|</span>
                                            <form id="delete-form-{{ $produk->id }}" action="{{ route('produk.destroy', $produk->id) }}" method="POST" class="inline">
                                                @csrf
                                                @method('DELETE')
                                                <button type="button" onclick="confirmDelete({{ $produk->id }})" class="text-red-600 hover:text-red-800 transition duration-200">Hapus</button>
                                            </form>
                                        </div>
                                    </td>
                                </tr>
                            @empty
                                <tr>
                                    <td colspan="6" class="px-6 py-4 text-center text-sm text-gray-500">Tidak ada data produk</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>

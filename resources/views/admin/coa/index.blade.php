...
<head>
    ...
    <title>Kelola Akun COA - Oriya Herbal Sejahtera</title>
    ...
    <script>
        ...
        function confirmDelete(id) {
            if (confirm('Apakah Anda yakin ingin menghapus akun COA ini?')) {
        ...
        <div class="flex-1 flex flex-col overflow-hidden">
            ...
            <main class="flex-1 overflow-y-auto bg-white p-6">
                <div class="flex items-center justify-between mb-6">
                    <h2 class="text-3xl font-bold text-[#2d5016]">Kelola Akun COA</h2>
                    <a href="{{ route('coa.create') }}" class="bg-[#90EE90] hover:bg-[#7ED87E] text-[#2d5016] px-4 py-2 rounded-lg font-semibold transition duration-200">
                        Tambah Akun
                    </a>
                </div>
                ...
                <div class="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
                    <table class="min-w-full divide-y divide-gray-200">
                        <thead class="bg-[#E8F5E9]">
                            <tr>
                                <th class="px-6 py-3 text-left text-xs font-medium text-[#2d5016] uppercase tracking-wider">Kode Akun</th>
                                <th class="px-6 py-3 text-left text-xs font-medium text-[#2d5016] uppercase tracking-wider">Nama Akun</th>
                                <th class="px-6 py-3 text-left text-xs font-medium text-[#2d5016] uppercase tracking-wider">Header Akun</th>
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            @forelse($coas as $coa)
                                <tr>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{{ $coa->kode_akun }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{{ $coa->nama_akun }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{{ $coa->header_akun }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                        <div class="flex items-center gap-2">
                                            <a href="{{ route('coa.edit', $coa->id) }}" class="text-[#2d5016] hover:text-[#90EE90] transition duration-200">Edit</a>
                                            <span class="text-gray-300">|</span>
                                            <form id="delete-form-{{ $coa->id }}" action="{{ route('coa.destroy', $coa->id) }}" method="POST" class="inline">
                                                @csrf
                                                @method('DELETE')
                                                <button type="button" onclick="confirmDelete({{ $coa->id }})" class="text-red-600 hover:text-red-800 transition duration-200">Hapus</button>
                                            </form>
                                        </div>
                                    </td>
                                </tr>
                            @empty
                                <tr>
                                    <td colspan="4" class="px-6 py-4 text-center text-sm text-gray-500">Tidak ada data COA</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>

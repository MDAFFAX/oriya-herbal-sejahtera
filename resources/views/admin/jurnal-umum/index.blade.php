...
<head>
    ...
    <title>Jurnal Umum - Oriya Herbal Sejahtera</title>
    ...
    <div class="flex h-screen overflow-hidden">
                ...
                <ul class="space-y-1">
                    ...
                    <li>
                        ...
                        <ul id="laporan" class="mt-1 ml-8 space-y-1">
                            <li>
                                <a href="{{ route('jurnal-umum.index') }}" class="block px-4 py-2 text-sm bg-[#90EE90] text-[#2d5016] rounded-lg font-semibold">Jurnal Umum</a>
                            </li>
                            <li>
        ...
        <div class="flex-1 flex flex-col overflow-hidden">
            ...
            <main class="flex-1 overflow-y-auto bg-white p-6">
                <div class="mb-6">
                    <h2 class="text-3xl font-bold text-[#2d5016] mb-1">Jurnal Umum</h2>
                    <p class="text-sm text-gray-600">Oriya Herbal Sejahtera</p>
                </div>

                <!-- Filter Form -->
                <form action="{{ route('jurnal-umum.index') }}" method="GET" class="mb-6 flex items-center gap-4">
                    <div class="flex-1 max-w-xs">
                        <label for="bulan" class="block text-sm font-medium text-gray-700 mb-2">Bulan</label>
                        <select name="bulan" id="bulan" 
                            class="w-full px-4 py-2 bg-[#E8F5E9] border border-[#C0E0C0] rounded-lg focus:outline-none focus:ring-2 focus:ring-[#90EE90] focus:border-transparent appearance-none">
                    ...
                    <div class="flex items-end">
                        <button type="submit" class="bg-[#90EE90] hover:bg-[#7ED87E] text-[#2d5016] px-6 py-2 rounded-lg font-semibold transition duration-200 h-[42px]">
                            Cari
                        </button>
                    </div>
                </form>
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            @forelse($jurnals as $jurnal)
                                <tr>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{{ $jurnal->nama_akun }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{{ $jurnal->ref }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{{ $jurnal->debet > 0 ? 'Rp ' . number_format($jurnal->debet, 0, ',', '.') : '-' }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{{ $jurnal->kredit > 0 ? 'Rp ' . number_format($jurnal->kredit, 0, ',', '.') : '-' }}</td>
                                </tr>
                            @empty
                                <tr>
